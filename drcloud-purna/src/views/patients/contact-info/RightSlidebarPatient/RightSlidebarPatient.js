...
import { getClinicPatientByIdAPI } from '@api/main'
...
const RightSlidebarPatient = ({
  ...
  useEffect(async () => {
    if (clinicPatientId) {
      try {
        const patientDetailRes = await getClinicPatientByIdAPI(clinicPatientId, xScreenId)
            ...
            <Can I='write' a='contacts'>
              <UncontrolledButtonDropdown className='slide-bar-header-btn' size='sm'>
                ...
                <DropdownMenu>
                  ...
                  <DropdownItem tag='a' onClick={() => handleDelete(patientDetail)}>
