...
import { getLocationsAPI } from '@api/main'
...
const LocationPermissionTab = ({ control, isAllLocations, isEditable }) => {
  ...
  // all location option
  const isAllLocation = [
    { label: 'label.allLocations', value: true },
    { label: 'label.limitedLocations', value: false }
  ]
      ...
      <UILoader blocking={loading}>
        <Controller
          ...
          render={({ onChange, value }) => (
            <FormGroup>
              {data?.map(l => (
                <div key={`locationIds[${l.locationId}]`}>
                  <hr className='my-50' />

                  <div
                    className={classnames('clinic-groups-permission-items', {
                      'item-disabled': isAllLocations
                    })}
                  >
                    <div className='d-flex align-items-center justify-content-between'>
                      <h6 className='item-name mb-0'>{l.locationName}</h6>
                        ...
                        <FormGroup className='mb-0 mr-2'>
                          ...
                          <div className='pl-50'>
                            <CustomInput
                              type='radio'
                              id={`${l.locationId}.yes`}
                              checked={!!value.find(id => id === l.locationId)}
                              onChange={e => {
                                onChange([...value, ...[l.locationId]].sort((a, b) => (a > b && 1) || -1))
                              }}
                              disabled={isAllLocations || !isEditable}
                            />
                          </div>
                        </FormGroup>
                        ...
                        <FormGroup className='mb-0 mr-1'>
                          ...
                          <div className='pl-50'>
                            <CustomInput
                              ...
                              checked={!value.find(id => id === l.locationId)}
                              onChange={e => {
                                const newIds = value.filter(id => id !== l.locationId)
                                onChange(newIds)
                              }}
                              disabled={isAllLocations || !isEditable}
                            />
...
export default LocationPermissionTab
